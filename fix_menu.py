...
# Remove the old node forward declarations
text = re.sub(r'static const MenuNode_t s_operation_channel;\nstatic const MenuNode_t s_channel_freq;\nstatic const MenuNode_t s_channel_txpower;', '', text)

# The handler replacements
# We'll just define the new generic handlers and the Render_OperationList
# Let's locate the block before "static int MenuKeyHandler_WorkMode(MenuKey_t key);"
handler_start = text.find('static int MenuKeyHandler_ChannelFreq(MenuKey_t key)')
handler_end = text.find('static void Render_SelfCheck(const MenuNode_t *node);')
...
text = text[:handler_start] + new_handlers + text[handler_end:]

# Now replace the old nodes with the new operation single list.
# We also have to define the rendering function for the list!
render_func = """
...
# Insert Render_OperationList and nodes
nodes_start = text.find('/* 叶子节点暂不指定 key_cb，后续在参数编辑时挂载 */')
nodes_end = text.find('/* --- FUNCTION 子菜单 --- */')
...
text = text[:nodes_start] + render_func + new_nodes + text[nodes_end:]

# We must also clean up the old WorkMode definition handler which might be lingering later if we didn't remove it properly. 
# Wait, I did text = text[:handler_start] + new_handlers + text[handler_end:], so anything in between is GONE.
# What was inside between handler_start and handler_end?
# Let's check what was between MenuKeyHandler_ChannelFreq and Render_SelfCheck.
# It contained MenuKeyHandler_WorkMode!
# And also MenuKeyHandler_UartBaud?
# Ah! I need to ensure MenuKeyHandler_UartBaud and MenuKeyHandler_OperationParam are preserved!
